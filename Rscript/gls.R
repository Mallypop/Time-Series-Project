...
dataseason.gls = gls(co2 ~ year + factor(seas), cor=corARMA( p=2, q=2)) #use lag order 2 and moving average from above 
...
par(mfrow=c(1,1))
ts.plot(cbind(ts.x, dataseason.gls$fitted, harmonizedARMA.gls$fitted), col=c(1,2,3), main="Compare mean monthly data with gls model")
...
anova.all ( harmonized.gls,harmonizedARMA.gls)
