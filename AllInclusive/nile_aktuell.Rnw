...
\section{The Nile River Time Series}%------------------------------------------------------------------------------------------------------
<<echo=FALSE>>=
library(tseries)
library(forecast)
library(mgcv)
...
kpss.test(Nile)
@
The data is not stationary, we can conclude that the moving average aproach was not appropriated.

\noindent We fit a linear model to the data
\begin{figure}
\centering
<<fig=TRUE>>=
...
\noindent The residuals look like a white noise, next we test if they are independent:
<<>>=
