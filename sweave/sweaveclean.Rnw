...
qqline(x, 
       col = 'red')
par(op)
mtext("CO2 Concentration (ppm) Histogram and QQ Plot", 
      line = 2.5, 
      font = 2, 
      cex = 0.8)
@
...
\begin{figure}[H]
\centering
<<label=fig1data.lm,fig=TRUE, echo=FALSE>>=
data = co2month[,c(3,5)]
...
pred=predict(data.lm, newdata=MyData, type="response", se=T) #poission: type ="LINK", binomial: type="RESPONSE"
...
In the plot of the data points in \ref{fig1data.lm} you can observe if you have outliers which could be biasing your model. \\
Also if you try to simply run a linear model, you see that a poly-1 linear regression will not fit (\ref{fig1data.lm}. You will get not the true evidence of p-values. The autocorrelation factor will bias your model results. The standard errors are less than  their true values thus giving high statistical significance with a p-value smaller than it should be. The clue in statistical modelling is to present the correct statistical evidence, which would in the case of a linear model highly biased. 
...
"There is a strong correlation at lag 1, a strong but weaker correlation at lag 2, and a noticeable correlation at lag 3. Such a correlation pattern is typical of an autoregressive process where most of
the sequential dependence can be explained as a  flow-on effect from a dependence at lag 1. In an autoregressive time series, an independent error component, or \innovation" is associated with each time point. For an order p autoregressive time series, the error for any time point is obtained
...
\sesction{Modelling the time series}
