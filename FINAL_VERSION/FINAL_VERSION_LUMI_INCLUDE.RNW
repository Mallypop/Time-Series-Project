...
library(TTR)
library(mgcv)
setwd("C:/Users/Mallypop/Desktop/Time-Series-Project/FINAL_VERSION")
@
...
dest<-"C:/Users/Mallypop/Desktop/Time-Series-Project/FINAL_VERSION/run.txt"
...
\begin{figure}[t]
 ...
 \begin{center}
\includegraphics[width=0.5\textwidth]{FINAL_VERSION-ACF_co2.pdf}
\label{correlogramlm}
\end{center}
\end{figure}
...
@
\includegraphics[width=0.4\textwidth]{FINAL_VERSIONa-correlogramres2.pdf}
}
\qquad
\subfloat[spectrum]{
...
\\
\noindent We can check if the data is stationary. We run a Durbin Watson Test to check for autocorrelations:
<<>>=
adf.test(Nile)
@
\\
...
\begin{center}
\subfloat[ACF]{
<<label=acf_nilelm, fig=TRUE, echo=FALSE, include=FALSE>>=
acf(resid(nilelm))
@
\includegraphics[width=0.4\textwidth]{acf_nilelm.pdf}
}
\qquad
\subfloat[PACF]{
<<label=pacf_nilelm, fig=TRUE, results=hide, include=FALSE>>=
pacf(resid(nilelm))
@
\includegraphics[width=0.4\textwidth]{pacf_nilelm.pdf}
}
\qquad
\subfloat[spectrum]{
<<label=spectrum_nilelm,fig=TRUE,results=hide, include=FALSE, echo=false>>=
spectrum(resid(nilelm), main = "")
@
\includegraphics[width=0.4\textwidth]{spectrum_nilelm.pdf}}
\end{center}
\caption{Correlogram of the residuals of nilelm}
\label{corr_res_lm}
\end{figure}
...
\begin{center}
\subfloat[ACF]{ 
<<label=acf_nilegls, fig=TRUE, echo=FALSE, include=FALSE>>=
acf(nilegls$residuals)
@
\includegraphics[width=0.4\textwidth]{acf_nilegls.pdf}
}
\qquad
\subfloat[PACF]{
 <<label=pacf_nilegls, fig=TRUE, echo=FALSE, include=FALSE>>=
pacf(nilegls$residuals)
@
\includegraphics[width=0.4\textwidth]{pacf_nilegls.pdf}}
\end{center}
...
The ACF does not show any significant autocorrelation.

\begin{figure}[h]
\centering
<<label=nilegls, fig=TRUE>>=
plot(nilegls)
@
\caption{Residuals of the gls Model}
\end{figure}

\noindent The residuals look like white noise, next we test if they are independent running a Durbin Watson Test to check for autocorrelations:\\
\\
<<>>=
...
begin{figure}[ht]
\begin{center}
\subfloat[HoltWinters]{ 
<<label=hwn_nile, fig=TRUE, include=FALSE>>=
library(forecast)
hwn = HoltWinters(Nile,alpha = 0.2, beta = FALSE, gamma = FALSE,
                  start.periods =c(1120,740) )
plot(hwn)
@
\includegraphics[width=0.4\textwidth]{hwn_nile.pdf}
}
\qquad
\subfloat[HWForecasting]{
 
<<label=forehw_nile, fig=TRUE, echo=FALSE, include=FALSE>>=
forecast_nile = forecast.HoltWinters(hwn, h=5)
plot.forecast(forecast_nile, shadecols = "oldstyle")
@
\includegraphics[width=0.4\textwidth]{forehw_nile.pdf}}
\end{center}
\end{figure}
...
\begin{center}
\subfloat[ACF]{ 
<<label=acf_hwnile, fig=TRUE, echo=FALSE, include=FALSE>>=
acf(forecast_nile$residuals, lag.max = 20)
@
\includegraphics[width=0.4\textwidth]{acf_hwnile.pdf}
}
\qquad
\subfloat[HWForecasting]{
 
<<label=pacfhw_nile, fig=TRUE, echo=FALSE, include=FALSE>>=
pacf(forecast_nile$residuals, lag.max = 20)
@
\includegraphics[width=0.4\textwidth]{pacfhw_nile}}
\end{center}
\end{figure}
...
\begin{center}
\subfloat[residuals]{ 
<<label=reshw, fig=TRUE, include=FALSE>>=
plot.ts(forecast_nile$residuals);abline(h=0)
@
\includegraphics[width=0.4\textwidth]{reshw.pdf}
}
\qquad
\subfloat[ForecastErrors]{
 <<label=for_errors, fig=TRUE, include=FALSE>>=
plotForecastErrors(forecast_nile$residuals)
@
\includegraphics[width=0.4\textwidth]{for_errors.pdf}}
\end{center}
...
\begin{center}
\subfloat[ACF]{ 
<<label=acf_arima, fig=TRUE, include=FALSE, echo=FALSE>>=
acf(forecast_nile2$residuals, lag.max = 20, main="")
@
\includegraphics[width=0.4\textwidth]{acf_arima.pdf}
}\
\qquad
\subfloat[PACF]{
<<label=pacf_arima, fig=TRUE, include=FALSE, echo=FALSE>>=
pacf(forecast_nile2$residuals, lag.max = 20, main="")
@
\includegraphics[width=0.4\textwidth]{pacf_arima.pdf}}
\end{center}
\caption{Correlograms for the Arima}
\end{figure}
...
\begin{center}
\subfloat[Residuals]{ 
<<label=res_nile2, fig=TRUE, include=FALSE, echo=FALSE>>=
plot.ts(forecast_nile2$residuals); abline(h=0)
@
\includegraphics[width=0.4\textwidth]{res_nile2.pdf}
}
\qquad
\subfloat[ForecastErrors]{
<<label=for_errors2, fig=TRUE, include=FALSE, echo=FALSE>>=
plotForecastErrors(forecast_nile2$residuals)
@
\includegraphics[width=0.4\textwidth]{for_errors2.pdf}}
\end{center}
\caption{Forecast Errors}
\end{figure}
...
<<>>=
fit_nile <- StructTS(Nile, type = "level")
fit_nile
@
\noindent The Maximum Likelihood Estimates (MLEs) of the level- and observation error variances, 1469 and 15099, respectively, are included in the output as the coefficients of fit_nile. 
\begin{figure}[ht]
\begin{center}
\subfloat[StructTS]{ 
<<label=fitsmooth_nile, fig=TRUE, include=FALSE>>=
plot(Nile)
# local level model (contemporaneous smoothing)
lines(fitted(fit_nile), lty = "dashed", col=4, lwd=2) 
# fixed-interval smoothing
lines(tsSmooth(fit_nile), lty = "dotted", col = 6, lwd=2) 
legend("bottomright" ,col = c(4,6),c("filtered","smoothed"),
       lty=c("dashed", "dotted"), bty="n", cex=0.8)
@
\includegraphics[width=0.4\textwidth]{fitsmooth_nile.pdf}
}
\qquad
\subfloat[Residuals]{
<<label=resid_ts, fig=TRUE, include=FALSE>>=
plot(residuals(fit_nile)); abline(h = 0, lty = 3)
# local trend model (constant trend fitted)
fit_nile2 <- StructTS(Nile, type = "trend")
@
\includegraphics[width=0.4\textwidth]{resid_ts.pdf}}
\end{center}
\end{figure}
...
library(forecast)
plot(forecast(fit_nile2, level = c(50,90), h = 10), xlim = c(1950, 1980), 
     shadecols = "oldstyle")
@
...
#values taken from the coefficients of fit_nile
